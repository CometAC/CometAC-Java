package ac.comet.cometac.platform.luckperms;
...
public abstract class AbstractLuckPermsHandler implements StoppableInitable {
    ...
    private static boolean isGrimPermission(PermissionNode node) {
        ...
        return permission.equals("*") || permission.equals("comet") || permission.startsWith("comet.");
    }
    ...
    private boolean inheritedGroupMayAffectGrimPermissions(InheritanceNode node) {
        ...
        // Refresh if the inherited group was deleted, or if that group can grant Comet permissions.
    ...
    private static void refreshGrimPlayerPermissions(UUID uuid) {
        if (uuid == null) return;
        CometPlayer player = CometAPI.INSTANCE.getPlayerDataManager().getPlayer(uuid);
