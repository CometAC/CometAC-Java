...
import ac.comet.cometac.utils.functions.ObjBooleanConsumer;
...
public class PacketPlayerJoinQuit extends PacketListenerAbstract {
    ...
    public void onPacketSend(PacketSendEvent event) {
        if (event.getPacketType() == PacketType.Login.Server.LOGIN_SUCCESS) {
            ...
            event.getTasksAfterSend().add(() -> {
                CometAPI.INSTANCE.getPlayerDataManager().addUser(event.getUser());
                ...
                if (uuid != null) {
                    CometAPI.INSTANCE.getDataStoreLifecycle().playerToggleStore().prefetch(uuid);
                }
            });
        }
    }
    ...
    public void onUserConnect(UserConnectEvent event) {
        ...
        if (event.getUser().getConnectionState() == ConnectionState.PLAY && !CometAPI.INSTANCE.getPlayerDataManager().isExemptUser(event.getUser())) {
    ...
    public void onUserLogin(UserLoginEvent event) {
        ...
        @NotNull PlatformPlayer platformPlayer = CometAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(nativePlayerObject);

        if (CometAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("debug-pipeline-on-join", false)) {
        ...
        applyToggle(platformPlayer, toggles, PlayerToggleStore.KEY_ALERTS,
                "cometac.alerts", "cometac.alerts.enable-on-join", "cometac.alerts.enable-on-join.silent",
                (p, silent) -> CometAPI.INSTANCE.getAlertManager().toggleAlerts(p, silent),
                (p, value) -> CometAPI.INSTANCE.getAlertManager().setAlertsEnabled(p, value, true));
        applyToggle(platformPlayer, toggles, PlayerToggleStore.KEY_VERBOSE,
                "cometac.verbose", "cometac.verbose.enable-on-join", "cometac.verbose.enable-on-join.silent",
                (p, silent) -> CometAPI.INSTANCE.getAlertManager().toggleVerbose(p, silent),
                (p, value) -> CometAPI.INSTANCE.getAlertManager().setVerboseEnabled(p, value, true));
        applyToggle(platformPlayer, toggles, PlayerToggleStore.KEY_BRANDS,
                "cometac.brand", "cometac.brand.enable-on-join", "cometac.brand.enable-on-join.silent",
                (p, silent) -> CometAPI.INSTANCE.getAlertManager().toggleBrands(p, silent),
                (p, value) -> CometAPI.INSTANCE.getAlertManager().setBrandsEnabled(p, value, true));

        if (platformPlayer.hasPermission("cometac.spectate") && CometAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("spectators.hide-regardless", false)) {
            CometAPI.INSTANCE.getSpectateManager().onLogin(platformPlayer.getUniqueId());
        }
    ...
    public void onUserDisconnect(UserDisconnectEvent event) {
        ...
        CometAPI.INSTANCE.getPlayerDataManager().onDisconnect(event.getUser());
    }
}
