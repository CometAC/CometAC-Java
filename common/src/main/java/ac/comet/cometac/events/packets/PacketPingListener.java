...
import ac.comet.cometac.utils.data.ShortToLongPair;
...
public class PacketPingListener extends PacketListenerAbstract {
    ...
    private static final GrimTransactionSendEvent.Channel SEND_CHANNEL = CometAPI.INSTANCE.getEventBus().get(GrimTransactionSendEvent.class);
    private static final GrimTransactionReceivedEvent.Channel RECEIVED_CHANNEL = CometAPI.INSTANCE.getEventBus().get(GrimTransactionReceivedEvent.class);
    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        if (event.getPacketType() == PacketType.Play.Client.WINDOW_CONFIRMATION) {
            ...
            if (id <= 0 && player.addTransactionResponse(id)) {
                ...
                boolean shouldCancel = !CometAPI.INSTANCE.getConfigManager().isDisablePongCancelling();
