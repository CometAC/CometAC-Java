...
import ac.comet.cometac.utils.blockplace.BlockPlaceResult;
import ac.comet.cometac.utils.blockplace.ConsumesBlockPlace;
import ac.comet.cometac.utils.change.BlockModification;
...
import ac.comet.cometac.utils.nmsutil.*;
...
public class CheckManagerListener extends PacketListenerAbstract {
    ...
    private static void placeWaterLavaSnowBucket(CometPlayer player, ItemStack held, StateType toPlace, InteractionHand hand, int sequence) {
    ...
    public static void handleQueuedPlaces(CometPlayer player, boolean hasLook, float pitch, float yaw, long now) {
    ...
    public static void handleQueuedBreaks(CometPlayer player, boolean hasLook, float pitch, float yaw, long now) {
    ...
    private static void handleUseItem(CometPlayer player, ItemStack placedWith, InteractionHand hand, int sequence) {
    ...
    private static void handleBlockPlaceOrUseItem(PacketWrapper<?> packet, CometPlayer player) {
    ...
    private static void placeBucket(CometPlayer player, InteractionHand hand, int sequence) {
    ...
    public static void setPlayerItem(CometPlayer player, InteractionHand hand, ItemType type) {
    ...
    private static void placeLilypad(CometPlayer player, InteractionHand hand, int sequence) {
        BlockHitData data = WorldRayTrace.getNearestBlockHitResult(player, null, true, false, true);
    ...
    private static boolean isMojangStupid(CometPlayer player, PacketReceiveEvent event, WrapperPlayClientPlayerFlying flying) {
        ...
        if (flying.hasPositionChanged() && flying.hasRotationChanged() &&
            ...
            // Additionally, only yaw/pitch matters: https://github.com/CometAnticheat/Comet/issues/1275#issuecomment-1872444018
    ...
    private static void handleFlying(CometPlayer player, double x, double y, double z, float yaw, float pitch, boolean hasPosition, boolean hasLook, boolean onGround, TeleportAcceptData teleportData, PacketReceiveEvent event) {
    ...
    private static void handleDigging(CometPlayer player, PacketReceiveEvent event) {
            ...
            if (damage >= 1) {
                ...
                player.blockHistory.add(
                        new BlockModification(
                                ...
                                blockBreak.position,
                                CometAPI.INSTANCE.getTickManager().currentTick,
