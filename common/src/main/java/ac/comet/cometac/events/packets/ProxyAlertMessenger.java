...
public class ProxyAlertMessenger extends PacketListenerAbstract {
    ...
    public ProxyAlertMessenger() {
        ...
        if (usingProxy) {
            ...
            CometAPI.INSTANCE.getPlatformServer().registerOutgoingPluginChannel("BungeeCord");
        }
    }
    ...
    public static void sendPluginMessage(String message) {
        ...
        Iterables.getFirst(CometAPI.INSTANCE.getPlatformPlayerFactory().getOnlinePlayers(), null).sendPluginMessage("BungeeCord", out.toByteArray());
    }

    public static boolean canSendAlerts() {
        return usingProxy && CometAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("alerts.proxy.send", false) && !CometAPI.INSTANCE.getPlatformPlayerFactory().getOnlinePlayers().isEmpty();
    }

    public static boolean canReceiveAlerts() {
        return usingProxy && CometAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("alerts.proxy.receive", false) && CometAPI.INSTANCE.getAlertManager().hasAlertListeners();
    }
