...
@CheckData(name = "MultiActionsC", stableKey = "cometac.multiactions.inventory_click_while_moving", verboseVersion = 1, description = "Clicked in inventory while moving")
public class MultiActionsC extends Check implements PacketCheck {
    ...
    public MultiActionsC(CometPlayer player) {
        super(player);
    }
    ...
    @Contract(pure = true)
    public static String getVerbose(@NotNull CometPlayer player) {
    ...
    @Contract(pure = true)
    public static boolean isVerboseSprinting(@NotNull CometPlayer player) {
        return player.isSprinting && (!player.isSwimming || !player.clientClaimsLastOnGround);
    }

    @Contract(pure = true)
    public static boolean isVerboseSneaking(@NotNull CometPlayer player) {
        return player.isSneaking && player.getClientVersion().isOlderThan(ClientVersion.V_1_15);
    }

    @Contract(pure = true)
    public static boolean isVerboseInput(@NotNull CometPlayer player) {
