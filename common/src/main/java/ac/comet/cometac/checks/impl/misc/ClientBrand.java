...
public class ClientBrand extends Check implements PacketCheck {
    ...
    public ClientBrand(CometPlayer player) {
        super(player);
    }
    ...
    private void handle(String channel, byte[] data) {
        ...
        if (data.length > 64 || data.length == 0) {
            ...
            if (!CometAPI.INSTANCE.getConfigManager().isIgnoredClient(brand)) {
                String message = CometAPI.INSTANCE.getConfigManager().getConfig().getStringElse("client-brand-format", "%prefix% &f%player% joined using %brand%");
                Component component = MessageUtil.replacePlaceholders(player, MessageUtil.miniMessage(message));

                CometAPI.INSTANCE.getAlertManager().sendBrand(component, null);
            }
            ...
            CometAPI.INSTANCE.getDataStoreLifecycle().liveWriteHooks().observeBrandFromCheck(player);
        }
        ...
        if (hasReachHacks && CometAPI.INSTANCE.getConfigManager().isBlockBlacklistedForgeClients()) {
            player.disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(player, CometAPI.INSTANCE.getConfigManager().getDisconnectBlacklistedForge())));
        }
