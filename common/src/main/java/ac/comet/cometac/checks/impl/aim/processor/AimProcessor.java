package ac.comet.cometac.checks.impl.aim.processor;
...
import ac.comet.cometac.utils.lists.RunningMode;
...
public class AimProcessor extends Check implements RotationCheck {
    ...
    public AimProcessor(CometPlayer playerData) {
        super(playerData);
    }
    ...
    public void process(final RotationUpdate rotationUpdate) {
        ...
        this.divisorX = CometMath.gcd(deltaXRot, lastXRot);
        if (deltaXRot > 0 && deltaXRot < 5 && divisorX > CometMath.MINIMUM_DIVISOR) {
        ...
        this.divisorY = CometMath.gcd(deltaYRot, lastYRot);

        if (deltaYRot > 0 && deltaYRot < 5 && divisorY > CometMath.MINIMUM_DIVISOR) {
