...
import ac.comet.cometac.utils.nmsutil.BlockBreakSpeed;
...
@CheckData(name = "FastBreak", stableKey = "cometac.breaking.fast_break", verboseVersion = 2, description = "Breaking blocks too quickly")
public class FastBreak extends Check implements BlockBreakCheck {
    ...
    public FastBreak(CometPlayer playerData) {
        super(playerData);
    }
    ...
    private void clampBalance() {
        ...
        blockBreakBalance = CometMath.clamp(blockBreakBalance, -balance, balance); // Clamp not Math.max in case other logic changes
        blockDelayBalance = CometMath.clamp(blockDelayBalance, -balance, balance);
    }
}
