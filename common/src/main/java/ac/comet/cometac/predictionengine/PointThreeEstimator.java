...
import ac.comet.cometac.utils.data.VelocityData;
 ...
 * Someone trying to false comet by using negative levitation effects
...
public class PointThreeEstimator {
    ...
    public PointThreeEstimator(CometPlayer player) {
        this.player = player;
    }
    ...
    public void handleChangeBlock(int x, int y, int z, WrappedBlockState state) {
        ...
        if (pointThreeBox.isIntersected(new SimpleCollisionBox(x, y, z))) {
            // https://github.com/MWHunter/Comet/issues/613
    ...
    private double iterateGravity(CometPlayer player, double y) {
