...
import ac.comet.cometac.utils.data.attribute.ValuedAttribute;
...
import ac.comet.cometac.utils.latency.CompensatedEntities;
...
import ac.comet.cometac.utils.nmsutil.FluidTypeFlowing;
...
public final class PlayerBaseTick {
    ...
    public static boolean canEnterPose(CometPlayer player, Pose pose, double x, double y, double z) {
        return Collisions.isEmpty(player, getBoundingBoxForPose(player, pose, x, y, z).expand(-1.0E-7D));
    }

    private static SimpleCollisionBox getBoundingBoxForPose(CometPlayer player, Pose pose, double x, double y, double z) {
    ...
    public static void doBaseTick(CometPlayer player) {
    ...
    private static void updateFluidOnEyes(CometPlayer player) {
    ...
    private static void updateInWaterStateAndDoFluidPushing(CometPlayer player) {
    ...
    public static void updatePowderSnow(CometPlayer player) {
    ...
    // 1.14
    public static void updatePlayerPose(CometPlayer player) {
    ...
    // 1.13 and below
    private static void updatePlayerSize(CometPlayer player) {
        Pose pose;
        if (player.isGliding) {
            pose = Pose.FALL_FLYING;
        } else if (player.isInBed) {
    ...
    private static void updateSwimming(CometPlayer player) {
    ...
    private static void moveTowardsClosestSpace(CometPlayer player, double xPosition, double zPosition) {
    ...
    private static void moveTowardsClosestSpaceLegacy(CometPlayer player, double x, double z) {
        int floorX = CometMath.floor(x);
        int floorZ = CometMath.floor(z);
        int floorY = CometMath.floor(player.lastY + 0.5);
    ...
    // 1.14+
    private static void moveTowardsClosestSpaceModern(CometPlayer player, double xPosition, double zPosition) {
    ...
    public static void updateInWaterStateAndDoWaterCurrentPushing(CometPlayer player) {
        final PacketEntity riding = player.compensatedEntities.self.getRiding();
    ...
    private static boolean updateFluidHeightAndDoFluidPushing(CometPlayer player, FluidTag tag, double multiplier) {
    ...
    private static boolean updateFluidHeightAndDoFluidPushingLegacy(CometPlayer player, FluidTag tag, double multiplier) {
        SimpleCollisionBox aABB = player.boundingBox.copy().expand(0, -0.4, 0).expand(-0.001);

        int floorX = CometMath.floor(aABB.minX);
        int ceilX = CometMath.ceil(aABB.maxX);
        int floorY = CometMath.floor(aABB.minY);
        int ceilY = CometMath.ceil(aABB.maxY);
        int floorZ = CometMath.floor(aABB.minZ);
        int ceilZ = CometMath.ceil(aABB.maxZ);
    ...
    private static boolean updateFluidHeightAndDoFluidPushingModern(CometPlayer player, FluidTag tag, double multiplier) {
    ...
    public static boolean updateFluidInteraction(CometPlayer player) {
    ...
    private static boolean suffocatesAt(CometPlayer player, int x, int z) {
    ...
    private static boolean clearAbove(CometPlayer player, int x, int y, int z) {
