...
import ac.comet.cometac.utils.enums.BoatEntityStatus;
...
public class PredictionEngineBoat extends PredictionEngine {
    public PredictionEngineBoat(CometPlayer player) {
    ...
    private static BoatEntityStatus getStatus(CometPlayer player) {
    ...
    private static @Nullable BoatEntityStatus isUnderwater(@NotNull CometPlayer player) {
        SimpleCollisionBox box = player.boundingBox;
        double maxBoxY = box.maxY + 0.001D;
        int minX = CometMath.floor(box.minX);
        int maxX = CometMath.ceil(box.maxX);
        int minY = CometMath.floor(box.maxY);
        int maxY = CometMath.ceil(maxBoxY);
        int minZ = CometMath.floor(box.minZ);
        int maxZ = CometMath.ceil(box.maxZ);
    ...
    private static boolean checkInWater(CometPlayer cometPlayer) {
        SimpleCollisionBox box = cometPlayer.boundingBox;
        int minX = CometMath.floor(box.minX);
        int maxX = CometMath.ceil(box.maxX);
        int minY = CometMath.floor(box.minY);
        int maxY = CometMath.ceil(box.minY + 0.001D);
        int minZ = CometMath.floor(box.minZ);
        int maxZ = CometMath.ceil(box.maxZ);
        boolean inWater = false;
        cometPlayer.vehicleData.waterLevel = -Double.MAX_VALUE;

        for (int x = minX; x < maxX; ++x) {
            for (int y = minY; y < maxY; ++y) {
                for (int z = minZ; z < maxZ; ++z) {
                    double level = cometPlayer.compensatedWorld.getWaterFluidLevelAt(x, y, z);
                    if (level > 0) {
                        float f = (float) ((float) y + level);
                        cometPlayer.vehicleData.waterLevel = Math.max(f, cometPlayer.vehicleData.waterLevel);
    ...
    public static float getGroundFriction(CometPlayer player) {
    ...
    private void floatBoat(CometPlayer player, Vector3dm vector) {
    ...
    public float getWaterLevelAbove(CometPlayer player) {
    ...
    private void controlBoat(CometPlayer player, Vector3dm vector, boolean intermediate) {
        float f = 0.0F;
        ...
        vector.add(new Vector3dm(player.trigHandler.sin(CometMath.radians(-player.yaw)) * f, 0, (double) (player.trigHandler.cos(CometMath.radians(player.yaw)) * f)));
    }
}
