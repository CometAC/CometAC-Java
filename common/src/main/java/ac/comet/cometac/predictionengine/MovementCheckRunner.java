...
import ac.comet.cometac.checks.impl.vehicle.VehicleC;
import ac.comet.cometac.checks.type.PositionCheck;
import ac.comet.cometac.manager.SetbackTeleportUtil;
import ac.comet.cometac.player.CometPlayer;
import ac.comet.cometac.predictionengine.movementtick.MovementTickerCamel;
import ac.comet.cometac.predictionengine.movementtick.MovementTickerHappyGhast;
import ac.comet.cometac.predictionengine.movementtick.MovementTickerHorse;
import ac.comet.cometac.predictionengine.movementtick.MovementTickerNautilus;
import ac.comet.cometac.predictionengine.movementtick.MovementTickerPig;
import ac.comet.cometac.predictionengine.movementtick.MovementTickerPlayer;
import ac.comet.cometac.predictionengine.movementtick.MovementTickerStrider;
import ac.comet.cometac.predictionengine.predictions.PredictionEngineNormal;
import ac.comet.cometac.predictionengine.predictions.rideable.PredictionEngineBoat;
import ac.comet.cometac.predictionengine.predictions.rideable.PredictionEngineRideableUtils;
...
import ac.comet.cometac.utils.data.SetBackData;
import ac.comet.cometac.utils.data.VectorData;
import ac.comet.cometac.utils.data.packetentity.PacketEntity;
import ac.comet.cometac.utils.data.packetentity.PacketEntityCamel;
...
import ac.comet.cometac.utils.data.packetentity.PacketEntityTrackXRot;
...
import ac.comet.cometac.utils.nmsutil.BoundingBoxSize;
...
public class MovementCheckRunner extends Check implements PositionCheck {
    ...
    public MovementCheckRunner(CometPlayer player) {
        super(player);
    }
    ...
    private void handleTeleport(PositionUpdate update) {
        ...
        //
        // However, for some reason, this behaviour does not work correctly in Comet
    ...
    private void check(PositionUpdate update) {
        ...
        if (!player.isFlying && player.isSneaking && Collisions.isAboveGround(player)) {
            ...
            double posX = Math.max(0.05, CometMath.clamp(player.actualMovement.getX(), -16, 16) + 0.05);
            double posZ = Math.max(0.05, CometMath.clamp(player.actualMovement.getZ(), -16, 16) + 0.05);
            double negX = Math.min(-0.05, CometMath.clamp(player.actualMovement.getX(), -16, 16) - 0.05);
            double negZ = Math.min(-0.05, CometMath.clamp(player.actualMovement.getZ(), -16, 16) - 0.05);
        ...
        if (player.vehicleData.wasVehicleSwitch || player.vehicleData.lastDummy) {
            ...
            if (riding != null) {
                ...
                // their similar code may also work on comet.
                //
