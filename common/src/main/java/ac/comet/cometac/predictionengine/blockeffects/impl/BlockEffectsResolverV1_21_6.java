...
public class BlockEffectsResolverV1_21_6 implements BlockEffectsResolver {
    ...
    private static void checkInsideBlocks(CometPlayer player, Vector3dm clientVelocity, boolean onlyApplyVelocity, Vector3d from, Vector3d to, LongSet visitedBlocks) {
    ...
    private static boolean forEachBlockIntersectedBetween(Vector3d start, Vector3d end, SimpleCollisionBox boundingBox, BlockStepVisitor blockStepVisitor) {
        ...
        if (direction.lengthSquared() < CometMath.square(0.99999F)) {
            ...
            if (iterationCount < 0) {
                ...
                for (Vector3i blockPos : SimpleCollisionBox.betweenClosed(boundingBox)) {
                    if (!alreadyVisited.contains(CometMath.asLong(blockPos)) && !blockStepVisitor.visit(blockPos, iterationCount + 1)) {
                        return false;
                    }
                }
    ...
    private static int addCollisionsAlongTravel(LongSet alreadyVisited, Vector3d start, Vector3d end, SimpleCollisionBox boundingBox, BlockStepVisitor blockStepVisitor) {
        ...
        while (tDeltaX <= 1.0 || tDeltaY <= 1.0 || tDeltaZ <= 1.0) {
            ...
            if (!collisionPoint.isEmpty()) {
                ...
                int endX = CometMath.floor(clampedX + boundingBox.getXSize());
                int endY = CometMath.floor(clampedY + boundingBox.getYSize());
                int endZ = CometMath.floor(clampedZ + boundingBox.getZSize());

                for (int x = currentX; x <= endX; x++) {
                    for (int y = currentY; y <= endY; y++) {
                        for (int z = currentZ; z <= endZ; z++) {
                            if (alreadyVisited.add(CometMath.asLong(x, y, z)) && !blockStepVisitor.visit(new Vector3i(x, y, z), iterationCount)) {
                                return -1;
                            }
                        }
                    }
