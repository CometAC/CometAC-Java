...
import ac.comet.cometac.predictionengine.PlayerBaseTick;
import ac.comet.cometac.predictionengine.predictions.PredictionEngine;
import ac.comet.cometac.predictionengine.predictions.PredictionEngineDualState;
import ac.comet.cometac.predictionengine.predictions.PredictionEngineElytra;
...
import ac.comet.cometac.utils.nmsutil.EntityTypeTags;
import ac.comet.cometac.utils.nmsutil.FluidFallingAdjustedMovement;
import ac.comet.cometac.utils.nmsutil.GetBoundingBox;
import ac.comet.cometac.utils.nmsutil.MainSupportingBlockPosFinder;
...
public class MovementTicker {
    public final CometPlayer player;

    public static void handleEntityCollisions(CometPlayer player) {
    ...
    public void move(Vector3dm inputVel, Vector3dm collide) {
        ...
        if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_18_2)) {
            boolean xAxis = !CometMath.equal(inputVel.getX(), collide.getX());
            boolean zAxis = !CometMath.equal(inputVel.getZ(), collide.getZ());
        ...
        if (collide.lengthSquared() <= 1e-7
            ...
            player.addMovementThisTick(new CometPlayer.Movement(from, to, new Vector3d(inputVel.getX(), inputVel.getY(), inputVel.getZ())));
        }
    ...
    public void livingEntityTravel() {
        ...
        double lavaLevel = 0;
        if (canStandOnLava())
            lavaLevel = player.compensatedWorld.getLavaFluidLevelAt(CometMath.floor(player.lastX), CometMath.floor(player.lastY), CometMath.floor(player.lastZ));
        ...
        if (player.wasTouchingWater && !player.isFlying) {
            ...
            player.friction = swimFriction; // Not vanilla, just useful for other comet stuff
            ...
            if (player.wasTouchingLava && !player.isFlying && !(lavaLevel > 0 && canStandOnLava())) {
                player.friction = 0.5F; // Not vanilla, just useful for other comet stuff
                ...
                if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_5) && Collisions.onClimbable(player, player.lastX, player.lastY, player.lastZ)) {
                    ...
                    player.friction = 0.99F; // Not vanilla, just useful for other comet stuff
