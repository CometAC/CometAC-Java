...
import ac.comet.cometac.predictionengine.EntityPushSimulator;
import ac.comet.cometac.predictionengine.SneakingEstimator;
import ac.comet.cometac.predictionengine.movementtick.MovementTickerPlayer;
...
import ac.comet.cometac.utils.nmsutil.JumpPower;
import ac.comet.cometac.utils.nmsutil.Riptide;
...
public class PredictionEngine {
    ...
    public static Vector3dm clampMovementToHardBorder(CometPlayer player, Vector3dm outputVel) {
        // TODO: Reimplement
        return outputVel;
    }

    public void guessBestMovement(float speed, CometPlayer player) {
    ...
    private void doPredictions(CometPlayer player, List<VectorData> possibleVelocities, float speed) {
    ...
    private Pair<Vector3dm, Vector3dm> doSeekingWallCollisions(CometPlayer player, Vector3dm primaryPushMovement, Vector3dm originalClientVel, VectorData clientVelAfterInput) {
    ...
    private void addZeroPointThreeToPossibilities(float speed, CometPlayer player, List<VectorData> possibleVelocities) {
    ...
    public void addFluidPushingToStartingVectors(CometPlayer player, Set<VectorData> data) {
    ...
    public Set<VectorData> fetchPossibleStartTickVectors(CometPlayer player) {
        ...
        // Inputs are done AFTER fluid pushing, https://github.com/MWHunter/Comet/issues/660
    ...
    private void addNonEffectiveAI(CometPlayer player, Set<VectorData> data) {
    ...
    private void addAttackSlowToPossibilities(CometPlayer player, Set<VectorData> velocities) {
    ...
    public void applyMovementThreshold(CometPlayer player, Set<VectorData> velocities) {
    ...
    public void addExplosionToPossibilities(CometPlayer player, Set<VectorData> existingVelocities) {
    ...
    public int sortVectorData(VectorData a, VectorData b, CometPlayer player) {
        int aScore = 0;
        int bScore = 0;
    ...
    public Vector3dm handleStartingVelocityUncertainty(CometPlayer player, VectorData vector, Vector3dm targetVec) {
        ...
        // A knockback velocity resync leaves a sub-0.003 Y that survives the client clamp but Comet reconstructs
        ...
        // https://github.com/MWHunter/Comet/issues/398
        ...
        //
        //
        // Comet's old solution with hard lerping entities was to just give a ton of direct offset reduction
    ...
    public void endOfTick(CometPlayer player, double d) {
        player.canSwimHop = canSwimHop(player);
        player.lastWasClimbing = 0;
    }

    private void loopVectors(CometPlayer player, Set<VectorData> possibleVectors, float speed, List<VectorData> returnVectors) {
        // Stop omni-sprint
    ...
    public boolean canSwimHop(CometPlayer player) {
    ...
    public Vector3dm handleOnClimbable(Vector3dm vector, CometPlayer player) {
        return vector;
    }

    public void doJump(CometPlayer player, Vector3dm vector) {
