...
public class CometVersion implements BuildableCommand {
    ...
    public static void checkForUpdatesAsync(Sender sender) {
        String current = CometAPI.INSTANCE.getExternalAPI().getGrimVersion();
        sender.sendMessage(Component.text()
                .append(Component.text("Comet Version: ").color(NamedTextColor.GRAY))
        ...
        lastCheck = now;
        CometAPI.INSTANCE.getScheduler().getAsyncScheduler().runNow(CometAPI.INSTANCE.getGrimPlugin(), () -> checkForUpdates(sender));
    }
    ...
    private static void checkForUpdates(Sender sender) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    ...
                    .GET()
                    .header("User-Agent", "CometAC/" + CometAPI.INSTANCE.getExternalAPI().getGrimVersion())
            ...
            if (statusCode < 200 || statusCode >= 300) {
                ...
                sender.sendMessage(Objects.requireNonNullElseGet(msg, () -> Component.text()
                        ...
                        .append(Component.text(" Failed to check latest CometAC version. Update server responded with status code: ")
            ...
            if (object.has("status")) {
                ...
                status = Status.SemVer.getVersionStatus(CometAPI.INSTANCE.getExternalAPI().getGrimVersion(), latest);
            }
            //
            Component msg = switch (status) {
                case AHEAD ->
                        Component.text("You are using a development version of CometAC").color(NamedTextColor.LIGHT_PURPLE);
                case UPDATED ->
                        Component.text("You are using the latest version of CometAC").color(NamedTextColor.GREEN);
                case OUTDATED -> Component.text()
                        .append(Component.text("New CometAC version found!").color(NamedTextColor.AQUA))
                        ...
                        .build();
                case UNKNOWN ->
                        Component.text("You are using an unknown CometAC version.").color(NamedTextColor.RED);
            };
            ...
            LogUtil.error("Failed to check latest CometAC version.", e);
        }
    }
