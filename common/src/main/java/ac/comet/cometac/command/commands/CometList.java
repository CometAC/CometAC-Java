...
public class CometList implements BuildableCommand {
    ...
    private void handleListPlayers(Sender sender) {
        ...
        final Map<UUID, PlatformPlayer> onlinePlayers = CometAPI.INSTANCE.getPlatformPlayerFactory().getOnlinePlayers()
        ...
        //
        for (CometPlayer entry : CometAPI.INSTANCE.getPlayerDataManager().getEntries()) {
            if (after) {
            ...
            boolean exempt = !CometAPI.INSTANCE.getPlayerDataManager().shouldCheck(entry.user);
