...
import ac.comet.cometac.checks.impl.badpackets.BadPacketsN;
...
public class SetbackTeleportUtil extends Check implements PostPredictionCheck {
    ...
    private static final GrimTeleportEvent.Channel TELEPORT_CHANNEL = CometAPI.INSTANCE.getEventBus().get(GrimTeleportEvent.class);
    private static final GrimPlayerSetbackEvent.Channel PLAYER_SETBACK_CHANNEL = CometAPI.INSTANCE.getEventBus().get(GrimPlayerSetbackEvent.class);
    ...
    public SetbackTeleportUtil(CometPlayer player) {
        super(player);
    }
        ...
        try {
            ...
            if (player.inVehicle()) {
                ...
                if (player.compensatedEntities.serverPlayerVehicle != null) {
                    ...
                    CometAPI.INSTANCE.getScheduler().getEntityScheduler().execute(player.platformPlayer, CometAPI.INSTANCE.getGrimPlugin(), () -> {
                        if (player.platformPlayer != null) {
                            CometEntity vehicle = player.platformPlayer.getVehicle();
                            if (vehicle != null) {
                                vehicle.eject();
                            }
                        }
            ...
            if (PacketEvents.getAPI().getServerManager().getVersion().isOlderThanOrEquals(ServerVersion.V_1_7_10)) {
                y += 1.62; // 1.7 teleport offset if comet ever supports 1.7 again
            }
    ...
    public boolean insideUnloadedChunk() {
        ...
        Column column = player.compensatedWorld.getChunk(CometMath.floor(player.x) >> 4, CometMath.floor(player.z) >> 4);
