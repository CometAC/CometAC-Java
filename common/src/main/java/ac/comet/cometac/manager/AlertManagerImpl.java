...
public final class AlertManagerImpl implements AlertManager, ConfigReloadable, StartableInitable {
    ...
    @Override
    public void start() {
        platformServer = CometAPI.INSTANCE.getPlatformServer();
     ...
     * @throws IllegalArgumentException if the user is not a CometPlayer.
     * @throws NullPointerException if the CometPlayer's platformPlayer is null.
     */
    private @NotNull PlatformPlayer requirePlatformPlayerFromUser(@NotNull GrimUser user) {
        ...
        if (!(user instanceof CometPlayer cometPlayer)) {
            // Throw a specific exception if the type is wrong
            throw new IllegalArgumentException("AlertManager action called with non-CometPlayer user: " + user.getName());
        }

        PlatformPlayer platformPlayer = cometPlayer.platformPlayer;
    ...
    public boolean hasBrandsEnabled(@NotNull GrimUser player) {
        CometPlayer cometPlayer = (CometPlayer) player;
        ...
        if (cometPlayer.platformPlayer == null) return false;

        return hasBrandsEnabled(cometPlayer.platformPlayer);
    }
    ...
    public boolean hasAlertsEnabled(Player player) {
        ...
        return hasAlertsEnabled(CometAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public void toggleAlerts(Player player) {
        if (player == null) return;
        toggleAlerts(CometAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public boolean hasVerboseEnabled(Player player) {
        if (player == null) return false;
        return hasVerboseEnabled(CometAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public void toggleVerbose(Player player) {
        if (player == null) return;
        toggleVerbose(CometAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }
