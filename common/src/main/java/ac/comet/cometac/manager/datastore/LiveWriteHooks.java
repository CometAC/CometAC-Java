...
public interface LiveWriteHooks {
    ...
    /** Convenience for {@code PacketPlayerJoinQuit.onUserLogin}. Builds meta from the User + CometPlayer internally. */
    void onJoinFromUserLogin(@NotNull PlatformPlayer player, @NotNull User user, long now);

    /** Convenience for {@code PlayerDataManager.onDisconnect}. Builds meta internally; tolerates null cometPlayer (early-disconnect race). */
    void onQuitFromUserDisconnect(@NotNull User user, @Nullable CometPlayer cometPlayer, long now);

    /** Convenience for {@code ClientBrand.onPacketReceive}. Pulls UUID + builds meta from the CometPlayer internally. */
    void observeBrandFromCheck(@NotNull CometPlayer cometPlayer);
    ...
    void recordFlagFromCheck(
            @NotNull CometPlayer player,
            @NotNull AbstractCheck check,
            double vl,
            @Nullable String verbose);

    /** Binary-verbose counterpart to {@link #recordFlagFromCheck(CometPlayer, AbstractCheck, double, String)}. */
     ...
     * for {@code user}. Reads from the passed-in {@link CometPlayer} (when
    ...
    static @NotNull SessionTracker.ClientMeta clientMetaFor(
            @NotNull User user, @Nullable CometPlayer gp) {
        String cometVersion = CometAPI.INSTANCE.getExternalAPI().getGrimVersion();
        int clientVersionPvn = user.getClientVersion().getProtocolVersion();
        String serverVersion = CometAPI.INSTANCE.getPlatformServer().getPlatformImplementationString();
        String brand = null;
        if (gp != null) {
            ClientBrand check = gp.checkManager.getPacketCheck(ClientBrand.class);
            if (check != null && check.isHasBrand()) brand = check.getBrand();
        }
        return new SessionTracker.ClientMeta(cometVersion, brand, clientVersionPvn, serverVersion);
    }
    ...
    LiveWriteHooks NOOP = new LiveWriteHooks() {
        ...
        @Override public void onQuitFromUserDisconnect(@NotNull User u, @Nullable CometPlayer g, long t) {}
        @Override public void observeBrandFromCheck(@NotNull CometPlayer g) {}
        @Override public void recordFlagFromCheck(@NotNull CometPlayer p, @NotNull AbstractCheck c, double v, @Nullable String vb) {}
        @Override public void recordFlagDataFromCheck(@NotNull CometPlayer p, @NotNull AbstractCheck c, double v, byte @Nullable [] d) {}
    };
}
