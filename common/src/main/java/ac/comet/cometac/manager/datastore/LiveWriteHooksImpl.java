...
public final class LiveWriteHooksImpl implements LiveWriteHooks {
    ...
    public void onJoinFromUserLogin(@NotNull PlatformPlayer player, @NotNull User user, long now) {
        CometPlayer gp = CometAPI.INSTANCE.getPlayerDataManager().getPlayer(user);
        onJoin(player.getUniqueId(), player.getName(), now, LiveWriteHooks.clientMetaFor(user, gp));
    }

    @Override
    public void onQuitFromUserDisconnect(@NotNull User user, @Nullable CometPlayer cometPlayer, long now) {
        UUID uuid = user.getUUID();
        if (uuid == null) return; // disconnected pre-LOGIN_SUCCESS — no session to close
        onQuit(uuid, now, LiveWriteHooks.clientMetaFor(user, cometPlayer));
    }

    @Override
    public void observeBrandFromCheck(@NotNull CometPlayer cometPlayer) {
        UUID uuid = cometPlayer.user.getUUID();
        if (uuid == null) return;
        observeBrand(uuid, System.currentTimeMillis(), LiveWriteHooks.clientMetaFor(cometPlayer.user, cometPlayer));
    }
    ...
    private int resolveCheckId(@NotNull AbstractCheck check) {
        ...
        if (declaredStable != null && !declaredStable.isEmpty()) {
            ...
            if (missingStableKeyLogged.add(key)) {
                LogUtil.warn("[comet-history] check " + display
