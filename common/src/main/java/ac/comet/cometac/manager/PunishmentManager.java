...
import ac.comet.cometac.events.packets.ProxyAlertMessenger;
...
public class PunishmentManager implements ConfigReloadable {
    ...
    private static final CommandExecuteEvent.Channel COMMAND_CHANNEL = CometAPI.INSTANCE.getEventBus().get(CommandExecuteEvent.class);

    public PunishmentManager(CometPlayer player) {
        this.player = player;
    }
    ...
    public boolean handleAlert(CometPlayer player, String verbose, Check check) {
        String value = verbose == null ? "" : verbose;
        return handleAlert(player, () -> value, check);
    }

    public boolean handleAlert(CometPlayer player, Supplier<String> verbose, Check check) {
        ...
        for (PunishGroup group : groups) {
            if (group.checks.contains(check)) {
                ...
                for (ParsedCommand command : group.commands) {
                    ...
                    // Verbose that prints all flags: /cometac verbose subscribers get EVERY flag, not just thresholded
                    ...
                    if (command.command.equals("[alert]") && CometAPI.INSTANCE.getAlertManager().hasVerboseListeners()) {
                        sentDebug = true;
                        ...
                        verboseListeners = CometAPI.INSTANCE.getAlertManager().sendVerbose(MessageUtil.miniMessage(listenerCmd), null, player.user.getName());
                    }
                    if (violationCount >= command.threshold) {
                        ...
                        if (shouldRun) {
                            ...
                            switch (command.command) {
                                case "[webhook]" -> CometAPI.INSTANCE.getDiscordManager().sendAlert(player, renderedVerbose, check.getDisplayName(), vl);
                                case "[log]" -> {
                                ...
                                case "[alert]" -> {
                                    ...
                                    if (testMode) { // secret test mode
                                        ...
                                        CometAPI.INSTANCE.getAlertManager().sendAlert(message, verboseListeners, player.user.getName());
                                    }
                                }
                                default -> CometAPI.INSTANCE.getScheduler().getGlobalRegionScheduler().run(CometAPI.INSTANCE.getGrimPlugin(), () ->
                                        CometAPI.INSTANCE.getPlatformServer().dispatchCommand(
                                                CometAPI.INSTANCE.getPlatformServer().getConsoleSender(),
                                                cmd
                                        )
                                );
                            }
