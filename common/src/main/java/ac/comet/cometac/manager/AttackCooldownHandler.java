...
public class AttackCooldownHandler extends Check implements PacketCheck {
    ...
    public AttackCooldownHandler(CometPlayer player) {
        super(player);
    }
    ...
    public float getMinimumProgress() {
        return CometMath.clamp(((float) ticksSinceLastSwing + 0.5F) / (float) (1d / player.compensatedEntities.self.getAttributeValue(Attributes.ATTACK_SPEED) * 20.0D), 0, 1);
    }
}
