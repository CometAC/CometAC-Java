package ac.comet.cometac.player;
...
import ac.comet.cometac.checks.impl.misc.ClientBrand;
import ac.comet.cometac.checks.impl.misc.TransactionOrder;
import ac.comet.cometac.checks.impl.packetorder.PacketOrderProcessor;
import ac.comet.cometac.events.packets.CheckManagerListener;
import ac.comet.cometac.manager.*;
import ac.comet.cometac.manager.player.features.FeatureManagerImpl;
import ac.comet.cometac.manager.player.handlers.DefaultResyncHandler;
import ac.comet.cometac.manager.player.handlers.NoOpResyncHandler;
import ac.comet.cometac.platform.api.player.PlatformPlayer;
import ac.comet.cometac.predictionengine.EntityFluidInteraction;
import ac.comet.cometac.predictionengine.MovementCheckRunner;
import ac.comet.cometac.predictionengine.PointThreeEstimator;
import ac.comet.cometac.predictionengine.UncertaintyHandler;
import ac.comet.cometac.manager.AttackCooldownHandler;
...
import ac.comet.cometac.utils.change.PlayerBlockHistory;
import ac.comet.cometac.utils.collisions.datatypes.SimpleCollisionBox;
import ac.comet.cometac.utils.data.*;
import ac.comet.cometac.utils.data.packetentity.PacketEntity;
import ac.comet.cometac.utils.data.packetentity.PacketEntityHappyGhast;
import ac.comet.cometac.utils.data.packetentity.PacketEntityNautilus;
import ac.comet.cometac.utils.data.packetentity.PacketEntitySelf;
import ac.comet.cometac.utils.data.tags.SyncedTags;
import ac.comet.cometac.utils.enums.FluidTag;
import ac.comet.cometac.utils.enums.Pose;
import ac.comet.cometac.utils.latency.*;
import ac.comet.cometac.utils.math.CometMath;
import ac.comet.cometac.utils.math.Location;
import ac.comet.cometac.utils.math.TrigHandler;
...
import ac.comet.cometac.utils.nmsutil.Materials;
...
public class CometPlayer implements GrimUser {
    ...
    public ac.comet.cometac.utils.inventory.InventoryDesyncStatus inventoryDesyncStatus = ac.comet.cometac.utils.inventory.InventoryDesyncStatus.NOT_DESYNCED;
    ...
    @Getter @Setter private ResyncHandler resyncHandler = CometAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("disable-default-resync-handler", false) ? NoOpResyncHandler.INSTANCE : new DefaultResyncHandler(this);
    ...
    // This variable is for support with test servers that want to be able to disable comet
    // Comet disabler 2022 still working!
    ...
    public CometPlayer(@NotNull User user) {
        this.user = user;
    ...
    public void onPacketCancel() {
        if (spamThreshold != -1 && cancelledPackets.incrementAndGet() > spamThreshold) {
            ...
            disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, CometAPI.INSTANCE.getConfigManager().getDisconnectClosed())));
    ...
    public void timedOut() {
        disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, CometAPI.INSTANCE.getConfigManager().getDisconnectTimeout())));
    }
    ...
    public void disconnect(Component reason) {
        ...
        if (platformPlayer != null) {
            CometAPI.INSTANCE.getScheduler().getEntityScheduler().execute(platformPlayer, CometAPI.INSTANCE.getGrimPlugin(),
    ...
    public void pollData() {
        ...
        if (!CometAPI.INSTANCE.getPlayerDataManager().shouldCheck(user)) {
            CometAPI.INSTANCE.getPlayerDataManager().remove(user);
        }
        ...
        if (uuid != null && this.platformPlayer == null) {
            this.platformPlayer = CometAPI.INSTANCE.getPlatformPlayerFactory().getFromUUID(uuid);
            updatePermissions();
        }
        ...
        if (uuid != null) {
            CometAPI.INSTANCE.getDataStoreLifecycle().sessionTracker()
    ...
    public void updatePermissions() {
        runSafely(() -> {
            try {
                boolean noModifyPacketPermission = hasPermission("cometac.nomodifypacket");
                boolean noSetbackPermission = hasPermission("cometac.nosetback");
                boolean disabledPermission = hasPermission("cometac.disabled");
                boolean exemptPermission = hasPermission("cometac.exempt");
    ...
    public int getTransactionPing() {
        return CometMath.floor(transactionPing / 1e6);
    }
    ...
    public final void reload(ConfigManager config) {
        ...
        maxTransactionTime = CometMath.clamp(config.getIntElse("max-transaction-time", 60), 1, 180);
    ...
    public void resyncPositions(SimpleCollisionBox box) {
        this.resyncHandler.resync(CometMath.floor(box.minX), CometMath.floor(box.minY), CometMath.floor(box.minZ),
                CometMath.ceil(box.maxX), CometMath.ceil(box.maxY), CometMath.ceil(box.maxZ));
    }

    public void addMovementThisTick(CometPlayer.Movement movement) {
        if (this.movementThisTick.size() >= 100) {
            CometPlayer.Movement movement1 = this.movementThisTick.removeFirst();
            CometPlayer.Movement movement2 = this.movementThisTick.removeFirst();
            CometPlayer.Movement movement3 = new CometPlayer.Movement(movement1.from(), movement2.to());
