...
import ac.comet.cometac.events.packets.PacketWorldBorder;
import ac.comet.cometac.player.CometPlayer;
import ac.comet.cometac.predictionengine.blockeffects.BlockEffectsResolver;
import ac.comet.cometac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_10;
import ac.comet.cometac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_2;
import ac.comet.cometac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_4;
import ac.comet.cometac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_5;
import ac.comet.cometac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_6;
import ac.comet.cometac.utils.chunks.Column;
...
import ac.comet.cometac.utils.data.ShulkerData;
import ac.comet.cometac.utils.data.VectorData;
import ac.comet.cometac.utils.data.tags.SyncedTags;
import ac.comet.cometac.utils.functions.BlockAndPositionConsumer;
import ac.comet.cometac.utils.functions.BlockAndPositionPredicate;
...
public final class Collisions {
    ...
    public static boolean slowCouldPointThreeHitGround(CometPlayer player, double x, double y, double z) {
    ...
    public static Vector3dm collide(CometPlayer player, double desiredX, double desiredY, double desiredZ) {
        return collide(player, desiredX, desiredY, desiredZ, desiredY, null);
    }

    public static Vector3dm collide(CometPlayer player, double desiredX, double desiredY, double desiredZ, double clientVelY, VectorData data) {
    ...
    public static boolean addWorldBorder(CometPlayer player, SimpleCollisionBox wantedBB, List<SimpleCollisionBox> listOfBlocks, boolean onlyCheckCollide) {
    ...
    public static boolean getCollisionBoxes(CometPlayer player, SimpleCollisionBox wantedBB, List<SimpleCollisionBox> listOfBlocks, boolean onlyCheckCollide) {
    ...
    private static CollisionBox getShulkerDynamicCollision(CometPlayer player, WrappedBlockState data, int x, int y, int z) {
    ...
    public static boolean isEmpty(CometPlayer player, SimpleCollisionBox playerBB) {
    ...
    public static Vector3dm maybeBackOffFromEdge(Vector3dm vec3, CometPlayer player, boolean overrideVersion) {
    ...
    public static boolean isAboveGround(CometPlayer player) {
    ...
    public static void handleInsideBlocks(CometPlayer player) {
        ...
        int minX = CometMath.mojangFloor(box.minX);
        int minY = CometMath.mojangFloor(box.minY);
        int minZ = CometMath.mojangFloor(box.minZ);
        int maxX = CometMath.mojangFloor(box.maxX);
        int maxY = CometMath.mojangFloor(box.maxY);
        int maxZ = CometMath.mojangFloor(box.maxZ);
    ...
    public static void onInsideBlock(CometPlayer player, Vector3dm clientVelocity, boolean onlyApplyVelocity, StateType blockType, WrappedBlockState block, int blockX, int blockY, int blockZ, boolean magic) {
    ...
    public static void applyEffectsFromBlocks(CometPlayer player) {
        ...
        player.stuckSpeedMultiplier = CometPlayer.DEFAULT_STUCK_SPEED;
        ...
        if (clientVersion.isOlderThan(ClientVersion.V_1_21_5)) {
            player.finalMovementsThisTick.add(new CometPlayer.Movement(from, to));
            ...
            if (player.finalMovementsThisTick.isEmpty()) {
                ...
                player.finalMovementsThisTick.add(new CometPlayer.Movement(player.finalMovementsThisTick.get(player.finalMovementsThisTick.size() - 1).to(), to));
            }
        }
    ...
    public static void resolveBlockEffects(CometPlayer player, Vector3d from, Vector3d to) {
        Collisions.resolveBlockEffects(player, List.of(new CometPlayer.Movement(from, to)));
    }

    public static void resolveBlockEffects(CometPlayer player, List<CometPlayer.Movement> movements) {
        resolveBlockEffects(player, player.clientVelocity, false, movements);
    }

    public static void resolveBlockEffects(CometPlayer player, Vector3dm clientVelocity, boolean onlyApplyVelocity, List<CometPlayer.Movement> movements) {
    ...
    private static double getOldDeltaY(CometPlayer player, double value) {
        return player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_2) ? value / 0.98F + 0.08 : value;
    }

    private static double getNewDeltaY(CometPlayer player, double value) {
        return player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_2) ? (value - 0.08) * 0.98F : value;
    }

    private static boolean isSlidingDown(Vector3dm vector, CometPlayer player, int locationX, int locationY,
                                         int locationZ) {
        if (player.onGround) {
            return false;
    ...
    // 0.03 hack
    public static boolean checkStuckSpeed(CometPlayer player, double expand) {
        // Use the bounding box for after the player's movement is applied
    ...
    public static boolean suffocatesAt(CometPlayer player, SimpleCollisionBox playerBB) {
    ...
    public static boolean doesBlockSuffocate(CometPlayer player, int x, int y, int z) {
        WrappedBlockState data = player.compensatedWorld.getBlock(x, y, z);
    ...
    // Thanks Tuinity
    public static boolean hasMaterial(CometPlayer player, SimpleCollisionBox checkBox, @NotNull BlockAndPositionPredicate searchingFor) {
    ...
    // Thanks Tuinity
    public static void forEachCollisionBox(@NotNull CometPlayer player, @NotNull SimpleCollisionBox checkBox, @NotNull BlockAndPositionConsumer searchingFor) {
    ...
    public static boolean onClimbable(CometPlayer player, double x, double y, double z) {
    ...
    public static boolean trapdoorUsableAsLadder(CometPlayer player, double x, double y, double z, WrappedBlockState blockData) {
