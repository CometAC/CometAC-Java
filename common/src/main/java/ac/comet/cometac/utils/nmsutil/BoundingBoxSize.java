...
import ac.comet.cometac.utils.data.packetentity.*;
...
public final class BoundingBoxSize {
    ...
    public static float getWidth(CometPlayer player, PacketEntity packetEntity) {
    ...
    private static float getWidthMinusBaby(CometPlayer player, PacketEntity packetEntity) {
    ...
    public static Vector3d getRidingOffsetFromVehicle(PacketEntity entity, CometPlayer player) {
        ...
        if (entity instanceof PacketEntityTrackXRot xRotEntity) {
            ...
            if (EntityTypes.isTypeInstanceOf(entity.getType(), EntityTypes.BOAT)) {
                ...
                vec3 = yRot(CometMath.radians(-xRotEntity.interpYaw) - ((float) Math.PI / 2f), vec3);
                return new Vector3d(x + vec3.x, y + (double) f1, z + vec3.z);
            } else if (entity.getType() == EntityTypes.LLAMA) {
                float f = player.trigHandler.cos(CometMath.radians(xRotEntity.interpYaw));
                float f1 = player.trigHandler.sin(CometMath.radians(xRotEntity.interpYaw));
                ...
                float f = player.trigHandler.sin(CometMath.radians(xRotEntity.interpYaw));
                float f1 = player.trigHandler.cos(CometMath.radians(xRotEntity.interpYaw));
    ...
    public static float getHeight(CometPlayer player, PacketEntity packetEntity) {
    ...
    public static double getPassengerRidingOffset(CometPlayer player, PacketEntity packetEntity) {
    ...
    private static float getHeightMinusBaby(CometPlayer player, PacketEntity packetEntity) {
        final EntityType type = packetEntity.getType();
    ...
    private static float getBabyScaleFactor(CometPlayer player, PacketEntity packetEntity) {
        final EntityType type = packetEntity.getType();
