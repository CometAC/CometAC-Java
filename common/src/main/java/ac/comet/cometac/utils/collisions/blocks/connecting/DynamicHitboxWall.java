package ac.comet.cometac.utils.collisions.blocks.connecting;
...
public class DynamicHitboxWall extends DynamicConnecting implements HitBoxFactory {
    @Override
    public CollisionBox fetch(CometPlayer player, StateType heldItem, ClientVersion version, WrappedBlockState state, boolean isTargetBlock, int x, int y, int z) {
    ...
    private int[] getConnections(CometPlayer player, ClientVersion version, WrappedBlockState state, int x, int y, int z) {
    ...
    @Override
    public boolean checkCanConnect(CometPlayer player, WrappedBlockState state, StateType one, StateType two, BlockFace direction) {
