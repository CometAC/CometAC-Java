...
public class FluidTypeFlowing {
    ...
    public static Vector3dm getFlow(CometPlayer player, int originalX, int originalY, int originalZ) {
    ...
    private static Vector3dm modern$getFlow(CometPlayer player, ClientVersion version, int originalX, int originalY, int originalZ) {
        ...
        for (BlockFace direction : new BlockFace[]{BlockFace.NORTH, BlockFace.EAST, BlockFace.SOUTH, BlockFace.WEST}) {
            ...
            if (affectsFlow(player, originalX, originalY, originalZ, modifiedX, originalY, modifiedZ)) {
                ...
                if (adjacentLevel == 0.0F) {
                    ...
                    // Comet's definition of solid is whether the block has a hitbox
    ...
    private static Vector3dm legacy$getFlow(CometPlayer player, ClientVersion version, int originalX, int originalY, int originalZ) {
        WrappedBlockState state = player.compensatedWorld.getBlock(originalX, originalY, originalZ);
    ...
    private static int legacy$getLiquidDepth(CometPlayer player, int x, int y, int z, boolean water) {
    ...
    private static boolean affectsFlow(CometPlayer player, int originalX, int originalY, int originalZ, int x2, int y2, int z2) {
        return isEmpty(player, x2, y2, z2) || isSame(player, originalX, originalY, originalZ, x2, y2, z2);
    }

    private static boolean isSolidFace(CometPlayer player, int originalX, int y, int originalZ, BlockFace direction) {
    ...
    public static boolean isEmpty(CometPlayer player, int x, int y, int z) {
    ...
    public static boolean isSame(CometPlayer player, int x1, int y1, int z1, int x2, int y2, int z2) {
