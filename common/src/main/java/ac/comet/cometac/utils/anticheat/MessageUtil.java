...
public class MessageUtil {
    ...
    public @Nullable String replacePlaceholders(@Nullable CometPlayer player, @Nullable String string, boolean removeFormatting) {
        return replacePlaceholders(player, player == null ? null : player.platformPlayer, string, removeFormatting);
    }

    @Contract("_, null -> null; _, !null -> !null")
    public @Nullable String replacePlaceholders(@Nullable CometPlayer player, @Nullable String string) {
    ...
    public @Nullable String replacePlaceholders(@Nullable PlatformPlayer player, @Nullable String string) {
        return replacePlaceholders(player == null ? null : CometAPI.INSTANCE.getPlayerDataManager().getPlayer(player.getUniqueId()), player, string, false);
    }
    ...
    private @Nullable String replacePlaceholders(@Nullable CometPlayer cometPlayer, @Nullable PlatformPlayer platformPlayer, @Nullable String string, boolean removeFormatting) {
        ...
        if (!matcher.find()) {
            return CometAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, string);
        }

        // Get references to the maps once, outside the loop.
        final Map<String, String> staticReplacements = CometAPI.INSTANCE.getExternalAPI().getStaticReplacements();
        final Map<String, Function<GrimUser, String>> variableReplacements = CometAPI.INSTANCE.getExternalAPI().getVariableReplacements();
        ...
        String cometReplaced = sb.toString();

        return CometAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, cometReplaced).replace(PLACEHOLDER_ESCAPE_CHAR, '%');
    }

    public @NotNull Component replacePlaceholders(@NotNull CometPlayer player, @NotNull Component component) {
    ...
    public @NotNull Component miniMessage(@NotNull String string) {
        string = string.replace("%prefix%", CometAPI.INSTANCE.getConfigManager().getConfig().getStringElse("prefix", "<gradient:#1565c0:#5cd6ff><bold>CometAC</bold></gradient> &8»"));
    ...
    public Component getParsedComponent(Sender sender, String key, String fallbackText) {
        String message = CometAPI.INSTANCE.getConfigManager().getConfig().getStringElse(key, fallbackText);
