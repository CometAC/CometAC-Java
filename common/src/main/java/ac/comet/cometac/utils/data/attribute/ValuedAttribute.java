package ac.comet.cometac.utils.data.attribute;
...
import static ac.comet.cometac.utils.latency.CompensatedEntities.SPRINTING_MODIFIER_UUID;
...
public final class ValuedAttribute {
     ...
     */
    public ValuedAttribute requiredVersion(CometPlayer player, ClientVersion requiredVersion) {
    ...
    public double with(WrapperPlayServerUpdateAttributes.Property property) {
        ...
        double newValue = CometMath.clamp((baseValue + additionSum) * (1 + multiplyBaseSum) * multiplyTotalProduct, min, max);
        if (setRewriter != null) {
