...
public class ReachUtils {
    ...
    @Contract("_, _, _ -> new")
    public static @NotNull Vector3dm getLook(@NotNull CometPlayer player, float yaw, float pitch) {
        if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_12_2)) {
            float yawRadians = CometMath.radians(-yaw) - (float) Math.PI;
            float pitchRadians = CometMath.radians(-pitch);
    ...
    public static double getMinReachToBox(@NotNull CometPlayer player, @NotNull SimpleCollisionBox targetBox) {
    ...
    public static @NotNull List<Vector3dm> getPossibleLookDirs(@NotNull CometPlayer player) {
    ...
    public static double getMinAngleToBox(@NotNull CometPlayer player, @NotNull SimpleCollisionBox box) {
