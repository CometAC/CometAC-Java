...
import ac.comet.cometac.utils.data.MainSupportingBlockData;
...
import ac.comet.cometac.utils.data.packetentity.PacketEntityStrider;
...
public class BlockProperties {
    public static float getFrictionInfluencedSpeed(float blockFriction, CometPlayer player) {
     ...
     */
    public static StateType getOnPos(CometPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static float getFriction(CometPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)) {
    ...
    public static float getBlockSpeedFactor(CometPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static boolean onHoneyBlock(CometPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
     ...
     */
    private static StateType getBlockPosBelowThatAffectsMyMovement(CometPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        Vector3i pos = player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)
                ? new Vector3i(CometMath.floor(playerPos.getX()), CometMath.floor(playerPos.getY() - 0.5000001), CometMath.floor(playerPos.getZ()))
                : getOnPos(player, playerPos, mainSupportingBlockData, 0.500001F);
        return player.compensatedWorld.getBlockType(pos.x, pos.y, pos.z);
    }

    private static Vector3i getOnPos(CometPlayer player, Vector3d playerPos, MainSupportingBlockData mainSupportingBlockData, float searchBelowPlayer) {
        ...
        if (mainBlockPos != null) {
            ...
            return shouldReturn ? mainBlockPos.withY(CometMath.floor(playerPos.getY() - (double) searchBelowPlayer)) : mainBlockPos;
        } else {
            return new Vector3i(CometMath.floor(playerPos.getX()), CometMath.floor(playerPos.getY() - searchBelowPlayer), CometMath.floor(playerPos.getZ()));
        }
    }

    public static float getMaterialFriction(CometPlayer player, StateType material) {
        float friction = 0.6f;
    ...
    private static StateType getOnBlock(CometPlayer player, double x, double y, double z) {
        StateType block1 = player.compensatedWorld.getBlockType(CometMath.floor(x), CometMath.floor(y - 0.2F), CometMath.floor(z));

        if (block1.isAir()) {
            StateType block2 = player.compensatedWorld.getBlockType(CometMath.floor(x), CometMath.floor(y - 1.2F), CometMath.floor(z));
    ...
    private static float getBlockSpeedFactorLegacy(CometPlayer player, Vector3d pos) {
    ...
    private static float getBlockSpeedFactor(CometPlayer player, StateType type) {
    ...
    private static float getModernVelocityMultiplier(CometPlayer player, float blockSpeedFactor) {
        if (player.getClientVersion().isOlderThan(ClientVersion.V_1_21)) return blockSpeedFactor;
        return (float) CometMath.lerp((float) player.compensatedEntities.self.getAttributeValue(Attributes.MOVEMENT_EFFICIENCY), blockSpeedFactor, 1.0F);
    }
}
