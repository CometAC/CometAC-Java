package ac.comet.cometac.utils.anticheat.update;

import ac.comet.cometac.player.CometPlayer;
import ac.comet.cometac.utils.anticheat.LogUtil;
import ac.comet.cometac.utils.collisions.AxisSelect;
...
public class BlockPlace {
    ...
    public BlockPlace(CometPlayer player, InteractionHand hand, Vector3i position, int faceId, BlockFace face, ItemStack itemStack, @Nullable BlockHitData hitData, int sequence) {
        this.player = player;
        this.hand = hand;
    ...
    private List<BlockFace> getNearestLookingDirections() {
        float pitch = CometMath.radians(player.pitch);
        float yaw = CometMath.radians(-player.yaw);
    ...
    public BlockFace getPlayerFacing() {
        return BY_2D[CometMath.floor(player.yaw / 90.0D + 0.5D) & 3];
    }
