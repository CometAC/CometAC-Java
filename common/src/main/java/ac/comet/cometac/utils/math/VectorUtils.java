...
public class VectorUtils {
    ...
    public static @NotNull Vector3dm cutBoxToVector(double x, double y, double z, @NotNull SimpleCollisionBox box) {
        return new Vector3dm(CometMath.clamp(x, box.minX, box.maxX),
                CometMath.clamp(y, box.minY, box.maxY),
                CometMath.clamp(z, box.minZ, box.maxZ));
    }
    ...
    public static @NotNull Vector3d clampVector(@NotNull Vector3d toClamp) {
        double x = CometMath.clamp(toClamp.getX(), -3.0E7D, 3.0E7D);
        double y = CometMath.clamp(toClamp.getY(), -2.0E7D, 2.0E7D);
        double z = CometMath.clamp(toClamp.getZ(), -3.0E7D, 3.0E7D);

        return new Vector3d(x, y, z);
    }

    public static Vector3dm normalize(CometPlayer player, Vector3dm vec) {
