...
import ac.comet.cometac.utils.data.tags.SyncedTag;
import ac.comet.cometac.utils.data.tags.SyncedTags;
import ac.comet.cometac.utils.enums.FluidTag;
import ac.comet.cometac.utils.inventory.EnchantmentHelper;
...
public class BlockBreakSpeed {
    // temporary hardcode to workaround PE bug https://github.com/retrooper/packetevents/issues/1217; see https://github.com/CometAnticheat/Comet/issues/2117
    ...
    // another temporary hardcode for the same reasons as above; see https://github.com/CometAnticheat/Comet/issues/2574
    ...
    public static double getBlockDamage(CometPlayer player, WrappedBlockState block) {
        ItemStack tool = player.inventory.getHeldItem();
        return getBlockDamage(player, tool, block.getType());
    }

    public static double getBlockDamage(CometPlayer player, ItemStack tool, StateType block) {
        // GET destroy speed
        ...
        final boolean canHarvest = !block.isRequiresCorrectTool() || toolSpeedData.isCorrectToolForDrop
                // temporary hardcode to workaround PE bug https://github.com/retrooper/packetevents/issues/1217; see https://github.com/CometAnticheat/Comet/issues/2091
                || player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_4) && HARVESTABLE_TYPES_1_21_4.contains(block)
                // same reason as above; see https://github.com/CometAnticheat/Comet/issues/2574
    ...
    private static float getSpeedMultiplierFromToolData(CometPlayer player, ItemStack tool, ToolSpeedData data) {
    ...
    private static ToolSpeedData getModernToolSpeedData(CometPlayer player, ItemStack tool, StateType block) {
    ...
    private static ToolSpeedData getLegacyToolSpeedData(CometPlayer player, ItemStack tool, StateType block) {
        ItemType toolType = tool.getType();
        float speedMultiplier = 1.0f;
        boolean isCorrectToolForDrop = false;
