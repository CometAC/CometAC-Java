package ac.comet.cometac.platform.bukkit.command;
...
public class BukkitPlayerSelectorAdapter implements PlayerSelector {
    ...
    public Sender getSinglePlayer() {
        return ((BukkitSenderFactory) CometAPI.INSTANCE.getSenderFactory()).map(bukkitSelector.single());
    }

    @Override
    public Collection<Sender> getPlayers() {
        return Collections.singletonList(((BukkitSenderFactory) CometAPI.INSTANCE.getSenderFactory()).map(bukkitSelector.single()));
    }
